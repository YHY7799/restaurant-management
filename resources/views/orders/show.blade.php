            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div>
                    ...
                    <p class="text-gray-600">
                        {{ $order->customer_name ?? ($order->customer ? $order->customer->name : 'Guest Customer') }}
                    </p>
                </div>
                <div>
                    <h2 class="text-lg font-semibold text-gray-700">Status</h2>
                    <p class="text-sm capitalize px-2 py-1 rounded-lg {{ $order->status->color() }}">
                        {{ $order->status->value }}
                    </p>
                </div>
                <div>
                    <h2 class="text-lg font-semibold text-gray-700">Phone Number</h2>
                    <p class="text-gray-600">{{ $order->customer_phone ?? ($order->customer ? $order->customer->phone : 'No phone Number') }}</p>
                </div>
                <div>
